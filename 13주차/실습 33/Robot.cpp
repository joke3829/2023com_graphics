...
void Robot::Initialize(GLuint* shaderProgram, int n)
{
	...
	head.Init_texture("robot\\head.png", 1024, 1024);
	nose.Initialize(shader, "robot\\nose.obj");
	nose.Init_texture("default.png", 1024, 1024);
	body.Initialize(shader, "robot\\body.obj");
	body.Init_texture("robot\\body.png", 1024, 1024);
	for (int i = 0; i < 2; ++i) {	//왼쪽 오른쪽
		arm[i].Initialize(shader, "robot\\arm_leg.obj");
		arm[i].Init_texture("default.png", 1024, 1024);
		leg[i].Initialize(shader, "robot\\arm_leg.obj");
		leg[i].Init_texture("default.png", 1024, 1024);
	}
