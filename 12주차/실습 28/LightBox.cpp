...
void LightBox::Initialize(GLuint* shaderprogram, std::string filename)
{
	...
	trans = glm::translate(trans, glm::vec3(dis, 5, 0));
}
...
void LightBox::change_color()
{
	...
	if (change) {
		...
		change = true;
		lightColor = glm::vec3(urd(dre), urd(dre), urd(dre));
	}
}

void LightBox::lightPower(char ww)
{
	switch (ww) {
	case '+':
		if (lightColor.x < 1.0)
