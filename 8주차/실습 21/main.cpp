...
void main(int argc, char** argv)
{
	...
	for(Camera& c : camera)
		c.Initialize(&shaderProgramID);

	camera[1].UpdateCamera(0, 90, 50);
	camera[2].UpdateCamera(0, 0, 40);
...
void TimerF(int value)
{
	...
	if (c_a) {
		crane.rotate(4);
		camera[0].Pos_update(crane.loc_return());
		//camera[0].Control_camera(crane.loc_return(), crane.angle_return());
	}
	if (c_d) {
		crane.rotate(6);
		camera[0].Pos_update(crane.loc_return());
